    ...
    return(
        <div className="tiny-six-item">
            <TinySlider settings={settings}>
                {hostingServices.map((item:Services, index:number) =>{
                    return (
                        <div className="tiny-slide" key={index}>
                            <Link href="#" className="group bg-white dark:bg-slate-900 block rounded-md overflow-hidden relative shadow-sm dark:shadow-gray-800 m-2">
                                <span className="p-4 block bg-white dark:bg-slate-900 text-center">
                                    <Image src={item.image} width={64} height={64} className="rounded-full shadow-md mx-auto size-16 mb-3" alt="" />
