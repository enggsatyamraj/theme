        ...
        <div className="md:flex justify-center">
            <div className="lg:w-2/3 md:w-full">
                <div id="accordion-collapse" data-accordion="collapse" className="mt-6">
                    {
                        hostingData.map((data:HostingData, index:number) => {
                            return(
                                <div className="relative shadow-sm dark:shadow-gray-800 rounded-md overflow-hidden mt-4" key={index}>
                                    ...
                                    <button 
                                            ...
                                            className={`flex justify-between items-center p-5 w-full font-medium text-start ${activeIndex === data.id ? 'bg-gray-50 dark:bg-slate-800 text-indigo-600' : ''}`} 
                                            data-accordion-target={`#${data.target}`} 
                                            aria-expanded={activeIndex === data.id ? "true" : "false"} 
                                            aria-controls={data.target}
                                            >
                                            <span>{data.que}</span>
                                            <svg data-accordion-icon className={`${activeIndex === data.id ? "rotate-180" : "rotate-270" } size-4 shrink-01`}  fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
