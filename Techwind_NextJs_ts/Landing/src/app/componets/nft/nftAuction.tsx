...
type NFTProduct = {
    ...
    amount: string;
    like: string;
    date?: string; // Date can be undefined
    dataGroup: string;
    category: string;
    remaining?: {
      hours: number;
...
export default function NftAuctions(){
    const [productData, setProductData] = useState<NFTProduct[]>(nftProdectData);
    ...
    const calculateDays = (date?: string) => {
        if (!date) return { hours: 0, minutes: 0, seconds: 0, days: 0 }; // Handle undefined date
                            ...
                            return(
                                <div key={index} className="group relative overflow-hidden bg-white dark:bg-slate-900 rounded-md shadow-sm dark:shadow-gray-800 hover:shadow-lg dark:hover:shadow-gray-800 duration-500 ease-in-out">
                                    <div className="relative">
                                        ...
                                        {item.date ?  <div className="inline-flex items-center absolute bottom-0 start-0 ms-6 mb-6 text-lg ltr:bg-gradient-to-r rtl:bg-gradient-to-l to-indigo-600 from-fuchsia-600 text-white rounded-full px-3">
