...
export default function Page() {
    ...
    const currentImage = photoImages.map((img) => ({ src: img }))
            ...
            <section className="relative pt-8 md:pb-24 py-16">
                ...
                <div className="container relative md:mt-24 mt-16">
                    ...
                    <div className="columns-4 gap-2">
                        {photoImages.map((item:string,index:number)=>{
                            return(
                                <div key={index} className="mb-2" >
                                        ...
                                        <div className="absolute top-1/2 -translate-y-1/2 start-0 end-0 text-center opacity-0 group-hover:opacity-100 duration-500">
                                            <Link href="#" onClick={()=>handleImageClick(index)} className="size-9 inline-flex items-center justify-center tracking-wide align-middle duration-500 text-base text-center rounded-full bg-indigo-600 hover:bg-indigo-700 border border-indigo-600 hover:border-indigo-700 text-white lightbox"><FiCamera className='size-4'/></Link>
                                        </div>
                                    </div>
                                </div>
                            )
