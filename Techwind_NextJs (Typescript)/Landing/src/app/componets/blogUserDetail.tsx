...
export default function BlogUserDetail({className}:{className:string}){
        ...
        <div className={className}>
            <div className="sticky top-20">
                ...
                <div className="flex flex-col gap-3 mt-8"> 
                    {RecentPostData.map((data:RecentPost, index:number) => {
                        return(
                            <div className="flex " key={index}>
                                <Image src={data.Image} width={96} height={64} className="h-16 rounded-md shadow-sm dark:shadow-gray-800" alt="" />
                ...
                <ul className="list-none text-center mt-8">
                    {TagscloudItem.map((data:string,index:number) => {
                        return(
                            <li className="inline-block m-2" key={index}><Link href="#" className="px-3 py-1 text-slate-400 hover:text-white dark:hover:text-white bg-gray-50 dark:bg-slate-800 text-sm hover:bg-indigo-600 dark:hover:bg-indigo-600 rounded-md shadow-sm dark:shadow-gray-800 transition-all duration-500 ease-in-out">{data}</Link></li>
                        )
                    }
